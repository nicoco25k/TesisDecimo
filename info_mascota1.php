...
if (isset($_POST['id_mascota'])) {
    ...
    try {
        ...
        $sql = "
    SELECT id_mascotas,nombre_mascota, nombre_especie, nombre_sexo, nombre_edad, nombre_talla, nombre_color, nombre_desparasitacion, nombre_extremidades, nombre_pelaje, nombre_peso, nombre_raza, nombre_esterilizacion, nombre_vacuna, ruta_img_mascota, caracteristicas_de_comportamiento_mascota 
    ...
    AND tm.id_mascotas = :id_mascota AND id_estado_adopcion_mascota='1'";
        ...
        if ($stmt->rowCount() > 0) {
            ...
            //aca enviare los datos

            $sql = "SELECT id_usuarios FROM `tabla_usuarios` WHERE nickname_usuario = '$usuario'";

            foreach ($dbh->query($sql) as $row) {
                $id_usuario = $row['id_usuarios'];
            }
            $id_mascota = ucfirst($mascota['id_mascotas']);
...
<script>';
            ...
            // Aquí puedes agregar cualquier código JavaScript adicional que necesites

            echo 'function mostrarAlerta() {
    swal({
        title: "¿Estás seguro de enviar tu petición para adoptar a ' . ucfirst($mascota['nombre_mascota']) . '?",
        text: "Si es así presiona OK, de lo contrario, cancela esta acción.",
        icon: "warning",
        buttons: true,
        dangerMode: true,
    })
    .then((willDelete) => {';
            ...
            echo '  // Aquí puedes agregar cualquier código PHP adicional que deseas ejecutar solo cuando se confirme la acción en el navegador

    // Puedes enviar una petición AJAX para ejecutar el código PHP en el servidor
    $.ajax({
        url: "registrar_adopcion.php",
        type: "POST",
        data: {
        ...
        success: function(response) {
            swal("!Se ha registrado tu petición con éxito! \n\n Ahora debes esperar a que nos pongamos en conctacto con tigo por medio de tus datos para completar esta petición",
        ...
        error: function(xhr, status, error) {
            swal("Se ha cancelado tu petición", {
                icon: "error",
            });
        }
    });

  ...
  swal({
    ...
    if (willUpdate) {
      if (campos.password) {
        ...
        $.post(
          "comprobar_editar_clave_admin.php",
          postData,
          function (response) {
            let x = response;
            ...
            if (x == 995) {
              swal(
                "Error!",
                "Querido usuario, la contraseña que digitaste no cumple con los requisitos",
                "error",
              );
              ...
              swal(
                "Error!",
                "Querido usuario, no puedes digitar la misma contraseña, digita una diferente",
                "error",
              );
              $("#formulario").trigger("reset");
            } else if (x == 998) {
              swal(
                "Error!",
                "Querido usuario, las contraseñas no son iguales, digitalas de nuevo",
                "error",
              );
              $("#formulario").trigger("reset");
            } else if (x == 2309) {
